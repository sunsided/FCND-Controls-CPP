...
namespace SLR {
    ...
    public:
        ...
        // CHECKED
        static Quaternion FromEulerYPR(const T &y, const T &p, const T &r) {//NB! rotation: 3-2-1 yaw,pitch,roll
            Quaternion q;
        ...
        Mat3x3F RotationMatrix_IwrtB(void) const {
            ...
            Mat3x3F Sk_q = Mat3x3F::SkewSymmetric(
                    qvec);// Build the skew symmetric matrix of the imaginary part of quaternion
            return Mat3x3F::Ident() * (_q[0] * _q[0] - qvec.magSq()) + Mat3x3F::OuterProduct(qvec, qvec) * 2 +
                   Sk_q * 2 * _q[0];
        };
        ...
        void RotationMatrix_IwrtB(float *R) const {
            const float &q0 = -_q[0];
            const float r0 = q0 * q0;
            const float &q1 = _q[1];
            const float r1 = q1 * q1;
            const float &q2 = _q[2];
            const float r2 = q2 * q2;
            const float &q3 = _q[3];
            const float r3 = q3 * q3;
            ...
            switch (col) {
                case 0:
                    switch (row) {
                        ...
                        case 1:
                            return 2 * q1 * q2 - 2 * q0 * q3;
                        default:
                            return 2 * q1 * q3 + 2 * q0 * q2;
                    };
                case 1:
                    switch (row) {
                        case 0:
        ...
        // CHECKED
        inline Quaternion
        IntegrateBodyRate(const V3D pqr, const double dt) //body rates must be expressed in the body coordinate frame!
        {
        ...
        // CHECKED
        inline Quaternion IntegrateBodyRate_fast(const V3F pqr,
                                                 const float half_dt) //body rates must be expressed in the body coordinate frame!
        {
            // half-pqr-dt values
        ...
        // CHECKED
        inline Quaternion IntegrateBodyRate_fast(float p, float q, float r,
                                                 const float half_dt) //body rates must be expressed in the body coordinate frame!
        {
            // half-pqr-dt values
            p *= -half_dt;
