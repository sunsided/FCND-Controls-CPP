...
SLR::LineD Visualizer_GLUT::ScreenToPickVector(double x, double y) {
    float winY = (float) (viewport[3] -
                          y);            // Subtract The Current Mouse Y Coordinate From The Screen Height.
    float winX = (float) (x);
...
void Visualizer_GLUT::DrawTrajectories(shared_ptr<QuadDynamics> quad) {
    if (quad) {
        if (quad->controller && showRefTrajectory) {
            VisualizeTrajectory(quad->controller->trajectory, true, V3F(0, 1, 1), 1.f, V3F(.1f, .2f, 1), quad->color,
...
void Visualizer_GLUT::VisualizeQuadCopter(shared_ptr<QuadDynamics> quad) {
    ...
    if (showPropCommands) {
        ...
        _glDraw->DrawArrow(pos + fl, pos + fl + down * cmd.desiredThrustsN[0] / maxThrust,
                           FalseColorRGB(cmd.desiredThrustsN[0] / maxThrust));            // front left
        _glDraw->DrawArrow(pos + fr, pos + fr + down * cmd.desiredThrustsN[1] / maxThrust,
                           FalseColorRGB(cmd.desiredThrustsN[1] / maxThrust));    // front right
        _glDraw->DrawArrow(pos - fr, pos - fr + down * cmd.desiredThrustsN[2] / maxThrust,
                           FalseColorRGB(cmd.desiredThrustsN[2] / maxThrust));        // rear left
        _glDraw->DrawArrow(pos - fl, pos - fl + down * cmd.desiredThrustsN[3] / maxThrust,
                           FalseColorRGB(cmd.desiredThrustsN[3] / maxThrust));    // front right
    }
}

void
Visualizer_GLUT::VisualizeTrajectory(const Trajectory &traj, bool drawPoints, V3F color, float alpha, V3F pointColor,
                                     V3F curPointColor, V3F offset, int style) {
