...
void GLCube(V3F center, V3F dims, int cnt) {
    ...
    GLRectangle(center - V3F(0, 0, 1) * dims / 2.f, V3F(0, 0, -1), V3F(1, 0, 0), dims.y, dims.x, cnt,
                cnt);        // bottom

    GLRectangle(center + V3F(1, 0, 0) * dims / 2.f, V3F(1, 0, 0), V3F(0, 0, 1), dims.y, dims.z, cnt,
                cnt);        // front
    GLRectangle(center - V3F(1, 0, 0) * dims / 2.f, V3F(-1, 0, 0), V3F(0, 0, 1), dims.y, dims.z, cnt,
                cnt);        // back

    GLRectangle(center + V3F(0, 1, 0) * dims / 2.f, V3F(0, 1, 0), V3F(0, 0, 1), dims.x, dims.z, cnt,
                cnt);        // left
    GLRectangle(center - V3F(0, 1, 0) * dims / 2.f, V3F(0, -1, 0), V3F(0, 0, 1), dims.x, dims.z, cnt,
                cnt);        // right
}
...
void glCircle(float radius, int numSegments) {
    ...
    for (int i = 0; i < numSegments; i++) {
        glVertex3f(radius * sinf(i * F_PI * 2.f / (float) numSegments),
                   radius * cosf(i * F_PI * 2.f / (float) numSegments), 0);
    }
    glEnd();
}
