...
void DrawX3D(V3D markingColor, V3D bodyColor = V3D(.2, .2, .2), double alpha = 1, bool solidPart = true,
             bool transPart = true, GLUquadricObj *glQuadric = NULL);

void
DrawQuarterX3D(bool front, V3D markingColor, V3D bodyColor, double alpha, GLUquadricObj *glQuadric, float armLength);
...
namespace SLR {
    ...
    public:
        ...
        void
        DrawQuadrotor2(V3F pos, Quaternion<float> att, V3F color, V3F centerOffset, float centerScale, float armLength);
