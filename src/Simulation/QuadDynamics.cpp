...
void QuadDynamics::Run(float dt, float simulationTime, int &idum, V3F externalForceInGlobalFrame,
    ...
    while (remainingTimeToSimulate >
           0.000001) // Time intervals lower than that are just discarded (for speed of running)
    {
...
void QuadDynamics::Dynamics(float dt, float simTime, V3F external_force, V3F external_moment, int &idum) {
    ...
    for (int m = 0; m < 4; m++) {
        if (motorCmdsN(m) >= motorCmdsOld(m)) {
            motorCmdsN(m) = motorCmdsN(m) * ((float) dt / ((float) dt + (float) tauaUp)) +
                            motorCmdsOld(m) * ((float) tauaUp / ((float) dt + (float) tauaUp));
        } else {
            motorCmdsN(m) = motorCmdsN(m) * ((float) dt / ((float) dt + (float) tauaDown)) +
                            motorCmdsOld(m) * ((float) tauaDown / ((float) dt + (float) tauaDown));
        }
    }
